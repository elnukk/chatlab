...
// Reasoning-family models (o1/o3/o4, gpt-5.x) reject the legacy `max_tokens` and
// `temperature` params on the Chat Completions API: max_tokens must be sent as
// `max_completion_tokens`, and temperature only supports its default (1).
function usesReasoningParams(model) {
...
const openaiAdapter = {
  ...
  async chat({ model, systemMessage, messages, temperature = 0.7, maxTokens = 1024, apiKey }) {
    ...
    const request = { model, messages: allMessages };

    if (usesReasoningParams(model)) {
